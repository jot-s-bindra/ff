...
//----------------------------------------------------------------
async function generateEdgeCasesAndCode(code, directoryPath, language,original_filePath) {
...
Please provide the code without any comments or additional text, strictly following the appropriate testing format. Ensure that the code includes a main function for execution and avoid adding trailing backticks.`;
        ...
        fs.writeFile(newFilePath, generatedCode, async (err) => {
            ...
            runCodeFile(code, edgeCases, generatedCode, newFilePath, language,original_filePath);
        });
...
async function runCodeFile(code, edgeCases, generatedCode, filePath, language,original_filePath) {
    try {
        let command = '';
        if (language === 'python') {
        ...
        exec(command, async (error, stdout, stderr) => {
            ...
            if (language === 'python') {
                ...
                showResultsInWebView(edgeCases, generatedCode, logs, reasons, fixed_formatCode, fixed_test_code, filePath,code,original_filePath);
                ...
                showResultsInWebView(edgeCases, generatedCode, logs, reasons_j, fixed_formatCode_j, fixed_test_code, filePath,code,original_filePath);
                loadingIndicator.hide(); 
                ...
                showResultsInWebView(edgeCases, generatedCode, logs, reasons_ts, fixed_formatCode_ts, fixed_test_code, filePath,code,original_filePath);
                loadingIndicator.hide(); 
                ...
                showResultsInWebView(edgeCases, generatedCode, logs, reasons_go, fixed_formatCode_go, fixed_test_code, filePath,code,original_filePath);
                loadingIndicator.hide(); 
    ...
    function continueExtensionExecution() {
        ...
        vscode.window.showOpenDialog({
            ...
            vscode.window.showOpenDialog({
                ...
                const original_filePath = uri[0].fsPath;
                fs.readFile(original_filePath, 'utf-8', async (err, data) => {
                    if (err) {
                        vscode.window.showErrorMessage(`Error reading file: ${err.message}`);
                        loadingIndicator.hide(); 
                        return;
                    }
                    const newFileName = path.basename(original_filePath);
                    ...
                    fs.writeFile(newFilePath, data, async (err) => {
                        ...
                        vscode.window.showInformationMessage(`File ${newFilePath} created successfully.`);
                        await generateEdgeCasesAndCode(data, directoryPath, language,original_filePath);
                    });
                });
            });
        });
...
//                    WEBVIEW / FRONTEND
//----------------------------------------------------------------
function showResultsInWebView(edgeCases, generatedCode, logs, reasons_go, fixed_formatCode_go,  fixed_test_code, filePath,originalcode,original_filePath) {
    ...
    <script>
        ...
        applyButton.addEventListener('click', () => {
            ...
            vscode.postMessage({
                ...
                content: fixedCodePre.innerText,
            });
        });
    ...
    panel.webview.onDidReceiveMessage(message => {
        switch (message.command) {
            ...
            case 'applyFixedTestCode':
                vscode.window.showInformationMessage('Applying fixed code...');
                saveFixedTestCode(message.content, original_filePath);
                break;
            default:
